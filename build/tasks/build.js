/**
 * Special build task to handle various jDoc build requirements.
...
async function getOutputRollupOptions( {
	...
	const wrapper = wrapperSource.split( "// @CODE" );
...
export async function build( {
	...
	if ( amd != null ) {
		if ( amd ) {
			console.log( "Naming jDoc with AMD name: " + amd );
		} else {
	...
	const jDocFileContents = await read( "jdoc.js" );
	if ( include.length ) {
		...
		// Remove the jDoc export from the entry file, we'll use our own
		// custom wrapper.
		setOverride(
			inputOptions.input,
			jDocFileContents.replace( /\n*export \{ jDoc };\n*/, "\n" ) +
				includedImports
		);
	}
...
export async function buildDefaultFiles( {
	...
	await Promise.all( [
		build( { version, watch } ),
		build( { filename: "jdoc.slim.js", slim: true, version, watch } ),
		build( {
			dir: "dist-module",
			filename: "jdoc.module.js",
			esm: true,
			version,
			watch
		} ),
		...
		build( {
			filename: "jdoc.factory.js",
			factory: true,
			version,
			watch
		} ),
		build( {
			filename: "jdoc.factory.slim.js",
			slim: true,
			factory: true,
			version,
			watch
		...
		build( {
			dir: "dist-module",
			filename: "jdoc.factory.module.js",
			esm: true,
			factory: true,
			version,
			watch
		...
		build( {
			dir: "dist-module",
			filename: "jdoc.factory.slim.module.js",
			esm: true,
			slim: true,
			factory: true,
			version,
	...
	if ( watch ) {
		...
		return compareSize( {
			files: [
				"dist/jdoc.min.js",
				"dist/jdoc.slim.min.js",
				"dist-module/jdoc.module.min.js",
				"dist-module/jdoc.slim.module.min.js"
			]
		} );
	}
}
