...
const DIST_REPO = "https://github.com/bssyco/jdoc-dist.git";
const SRC_REPO = "https://github.com/bssyco/jdoc.git";
const CDN_URL = "https://github.com/bssyco/jDoc/dist";
const REGISTRY_URL = "https://registry.npmjs.org/jdoc";
...
async function verifyRelease() {
	...
	if ( !npmPackage.versions[ version ] ) {
		throw new Error( `jDoc ${ version } not found on npm!` );
	}
	...
	if ( verified ) {
		console.log( `jDoc ${ version } is reproducible! All files match!` );
	} else {
		console.log();
		...
		throw new Error( `jDoc ${ version } is NOT reproducible!` );
	}
}
...
async function buildRelease( { version } ) {
	...
	console.log( `Cloning jDoc ${ version }...` );
	...
	console.log( `Installing dependencies for jDoc ${ version }...` );
	...
	// Build the release
	console.log( `Building jDoc ${ version }...` );
	...
	// Clone the dist repo
	console.log( `Cloning jdoc-dist ${ version }...` );
	...
	console.log( `Preparing jDoc ${ version } for packaging...` );
	...
	console.log( `Packing jDoc ${ version }...` );
		...
		[ ...distFiles, ...distModuleFiles ]
			...
			.map( async( dirent ) => {
				...
				return {
					...
					contents,
					cdnName: dirent.name.replace( rjdoc, `jdoc-${ version }` ),
	...
	const tgzFilename = `jdoc-${ version }.tgz`;
...
function convertMapToVersioned( contents, version ) {
	...
	return JSON.stringify( {
		...map,
		file: map.file.replace( rjdoc, `jdoc-${ version }` ),
		sources: map.sources.map( ( source ) => source.replace( rjdoc, `jdoc-${ version }` ) )
	} );
}
